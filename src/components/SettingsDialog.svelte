...
{#if showDialog}
    <div class="settings" on:stopPropagation>
        ...
        <label class="setting-label">
            ...
            <select on:change={updateClockFormat}>
                {#each formatOptions as option}
                    <option value={JSON.stringify(option)} selected={option.label === clockFormatLabel}>{option.label}</option>
                {/each}
            </select>
        </label>
        <br />
        ...
        <label class="setting-label">
            Clock font size:
            <input type="range" min="10" max="550" value={parseInt(clockFontSize)} on:change="{updateClockFontSize}" />
        </label>
        <br />
        <label class="setting-label">
            Clock position X:
